# The `LIBLAMMPS`{.interpreted-text role="f:mod"} Fortran Module
...
A fully Fortran 2003 compatible Fortran compiler is required. This means
...
> This method will call `lammps_command`{.interpreted-text
...
>   -------------------- ------------------- ------------------------------------------- -----------------
>   `lmp%style%global`   `lmp%type%scalar`   `REAL(c_double), POINTER`                   Global scalar
...
>     *vector*-style variables, or `CHARACTER(LEN=*)` for *string*-style
...
> by atom ID (e.g., if *name* is *x* and *count* = 3, then *data* =
...
> This function calls `lammps_gather_atoms_subset`{.interpreted-text
...
> the array \[*x*(1,1), *x*(2,1), *x*(3,1), *x*(1,100), *x*(2,100),
...
> scatter data for all atoms, in order.
>
...
>     arranged as $[v_{1,x},v_{1,y},v_{1,z},v_{2,x}, \dotsc]$; if
...
> to
>
> :   `lammps_config_has_jpeg_support`{.interpreted-text
>     role="cpp:func"}
>
> r has_jpeg
>
...
> Check if the LAMMPS library supports creating movie files via a pipe
> to ffmpeg.
>
> ::: versionadded
> 3Nov2022
> :::
>
> The LAMMPS [dump style movie](dump_image) supports generating movies
> from images on-the-fly via creating a pipe to the
> [ffmpeg](https://ffmpeg.org/)\_ program. This function checks whether
> this feature was [enabled at compile time](graphics). It does **not**
> check whether the `ffmpeg` itself is installed and usable.
>
> to
>
...
> :::
>
> In case of an error, LAMMPS will either abort or throw a C++
> exception. The latter has to be [enabled at compile time](exceptions).
> This function checks if exceptions were enabled.
>
> When using the library interface with C++ exceptions enabled, the
> library interface functions will \"catch\" them, and the error status
> can then be checked by calling `has_error`{.interpreted-text
> role="f:func"}. The most recent error message can be retrieved via
> `get_last_error_message`{.interpreted-text role="f:func"}. This can
> allow one to restart a calculation or delete and recreate the LAMMPS
> instance when a C++ exception occurs. One application of using
> exceptions this way is the [lammps_shell]{.title-ref}. If C++
> exceptions are disabled and an error happens during a call to LAMMPS
> or the Fortran API, the application will terminate.
>
> to
>
...
> specific [LAMMPS package](Packages) provided as argument.
>
> to
>
...
> :::
>
> This function counts how many [LAMMPS packages](Packages) are included
> in the LAMMPS library in use. It directly calls the C library function
...
> :   `lammps_config_package_count`{.interpreted-text role="cpp:func"}
>
> r integer(c_int) npackages
>
> :   number of packages installed

------------------------------------------------------------------------

> Get the name of a package in the list of installed packages in the
> LAMMPS library.
>
> ::: versionadded
> 3Nov2022
> :::
>
> This subroutine copies the name of the package with the index *idx*
> into the provided string *buffer*. If the name of the package exceeds
> the length of the buffer, it will be truncated accordingly. If the
> index is out of range, *buffer* is set to an empty string.
>
> p integer(c_int) idx
>
> :   index of the package in the list of included packages
>     $(0 \le idx < \text{package count})$
>
> p character(len=\*) buffer
>
> :   string to hold the name of the package
>
> to
>
...
> Obtain a list of the names of enabled packages in the LAMMPS shared
> library and store it in *package*.
>
> ::: versionadded
> 3Nov2022
> :::
>
> This function is analogous to the `installed_packages
> <lammps.lammps.installed_packages>`{.interpreted-text
> role="py:meth"}\_\_ function in the Python API. The optional argument
> *length* sets the length of each string in the vector *package*
> (default: 31).
>
> :p character(len=:) package \[dimension(:),allocatable\]: list of packages;
>
> :   *must* have the `ALLOCATABLE` attribute and be of rank 1 (i.e.,
...
> o length
>
> :   length of each string in the list. Default: 31.
>
> otype length
>
> :   integer,optional
...
> This function calls `lammps_config_accelerator`{.interpreted-text
> role="cpp:func"} to check the availability of compile time settings of
> included [accelerator packages](Speed_packages) in LAMMPS.
>
> ::: versionadded
> 3Nov2022
> :::
>
> Supported packages names are \"GPU\", \"KOKKOS\", \"INTEL\", and
> \"OPENMP\". Supported categories are \"api\" with possible settings
> \"cuda\", \"hip\", \"phi\", \"pthreads\", \"opencl\", \"openmp\", and
> \"serial\"; and \"precision\" with possible settings \"double\",
> \"mixed\", and \"single\".
>
> p character(len=\*) package
>
> :   string with the name of the accelerator package
>
> p character(len=\*) category
>
> :   string with the name of the setting
>
> p character(len=\*) setting
>
> :   string with the name of the specific setting
>
> to
>
...
> r available
>
> :   `.TRUE.` if the combination of package, category, and setting is
...
> :::
>
> This function calls `lammps_has_gpu_device`{.interpreted-text
> role="cpp:func"}, which checks at runtime whether an accelerator
> device is present that can be used with the [GPU package](Speed_gpu).
>
> More detailed information about the available device or devices can be
> obtained by calling the `get_gpu_device_info`{.interpreted-text
> role="f:subr"} subroutine.
>
> to
>
...
> r available
>
> :   `.TRUE.` if a viable device is available, `.FALSE.` if not.
>
> rtype available
>
> :   logical
...
> :::
>
> Calls `lammps_get_gpu_device_info`{.interpreted-text role="cpp:func"}
> to retrieve detailed information about any accelerator devices that
> are viable for use with the [GPU package](Speed_gpu). It will fill
> *buffer* with a string that is equivalent to the output of the
> `nvc_get_device` or `ocl_get_device` or `hip_get_device` tools that
> are compiled alongside LAMMPS if the GPU package is enabled.
>
> A suitable-length Fortran string has to be provided. The assembled
> text will be truncated so as not to overflow this buffer. This string
> can be several kilobytes long if multiple devices are present.
>
> p character(len=\*) buffer
>
> :   string into which to copy the information.
>
> to
>
> :   `lammps_get_gpu_device_info`{.interpreted-text role="cpp:func"}
...
> :::
>
> This function calls `lammps_has_style`{.interpreted-text
> role="cpp:func"} to check whether the LAMMPS library in use includes
> the specific style *name* associated with a specific *category*
> provided as arguments. Please see `lammps_has_style`{.interpreted-text
> role="cpp:func"} for a list of valid categories.
>
> p character(len=\*) category
>
...
> `lammps_has_style`{.interpreted-text role="cpp:func"} for a list of
> valid categories.
>
> p character(len=\*) category
>
> :   category of styles to count
>
> to
>
> :   `lammps_style_count`{.interpreted-text role="cpp:func"}
>
> r integer(c_int) count
>
...
> :::
>
> This function calls `lammps_style_name`{.interpreted-text
> role="cpp:func"} and copies the name of the *category* style with
> index *idx* into the provided string *buffer*. The length of *buffer*
> must be long enough to contain the name of the style; if it is too
> short, the name will be truncated accordingly. If *idx* is out of
> range, *buffer* will be the empty string and a warning will be issued.
>
> p character(len=\*) category
>
> :   category of styles
...
> p integer(c_int) idx
>
> :   index of the style in the list of *category* styles
>     $(1 \leq idx \leq \text{style count})$
>
> p character(len\*) buffer
>
> :   string buffer to copy the name of the style into
>
> to
>
...
> r has_id
>
> :   `.TRUE.` if *category* style *name* exists, `.FALSE.` if not.
>
> rtype has_id
>
> :   logical
...
> This function counts how many IDs in the provided *category* are
> defined in the current LAMMPS instance. Please see
> `has_id`{.interpreted-text role="f:func"} for a list of valid
> categories.
>
> ::: versionadded
> 3Nov2022
...
> *idx* into the provided string *buffer*. The length of the buffer must
> be long enough to hold the string; if the name of the style exceeds
> the length of the buffer, it will be truncated accordingly. If
> *buffer* is `ALLOCATABLE`, it must be allocated *before* the function
> is called. If *idx* is out of range, *buffer* is set to an empty
> string and a warning is issued.
>
> p character(len=\*) category
>
> :   category of IDs
...
> p integer(c_int) idx
>
> :   index of the ID in the list of *category* styles
>     ($0 \leq idx < count$)
>
> p character(len=\*) buffer
>
> :   string into which to copy the name of the style
>
> to
>
...
> to
>
> :   `lammps_plugin_count`{.interpreted-text role="cpp:func"}
>
> r n
>
...
> Look up the style and name of a plugin by its index in the list of
> plugins.
>
> ::: versionadded
> 3Nov2022
...
> *idx* into the provided C-style string buffer. The length of the
> buffer must be provided as *buf_size* argument. If the name of the
> style exceeds the length of the buffer, it will be truncated
> accordingly. If the index is out of range, both strings are set to the
> empty string and a warning is printed.
>
> p integer(c_int) idx
>
> :   index of the plugin in the list all or *style* plugins
>
> p character(len=\*) stylebuf
>
> :   string into which to copy the style of the plugin
>
> p character(len=\*) namebuf
>
> :   string into which to copy the style of the plugin
>
> to
>
> :   `lammps_plugin_name`{.interpreted-text role="cpp:func"}
...
> representing the image flags in the $x$-, $y$-, and $z$-directions.
> Unless LAMMPS is compiled with `-DLAMMPS_BIGBIG`, those integers are
> limited to 10-bit signed integers $[-512,512)$. If `-DLAMMPS_BIGBIG`
> was used when compiling, then the return value is of kind `c_int64_t`
> instead of kind `c_int`, and the valid range for the individual image
> flags becomes $[-1048576,1048575)$ (i.e., the range of a 21-bit signed
> integer). There is no check on whether the arguments conform to these
> requirements; values out of range will simply be wrapped back into the
> interval.
>
> p integer(c_int) ix
>
> :   image flag in $x$-direction
>
> p integer(c_int) iy
>
> :   image flag in $y$-direction
>
> p integer(c_int) iz
>
> :   image flag in $z$-direction
>
> r integer(kind=\*) imageint
>
> :   encoded image flag. \*The `KIND` parameter is `c_int` unless
...
> return value until compile time means that it is impossible to define
> an interface that works for both sizes of `imageint`. One side effect
> of this is that you must assign the return value of this function to a
> variable; it cannot be used as the argument to another function or as
> part of an array constructor. For example,
>
> ``` fortran
...
> `encode_image_flags`{.interpreted-text role="f:func"}: it takes the
> image flag and performs the bit-shift and bit-masking operations to
> decode it and stores the resulting three integers into the array
> *flags*.
>
> ::: versionadded
> 3Nov2022
> :::
>
> p integer(kind=\*) image
>
> :   encoded image flag. \*The `KIND` parameter is either `c_int` or,
>     if LAMMPS was compiled with `-DLAMMPS_BIGBIG`, `c_int64_t`. Kind
>     compatibility is checked at run-time.
>
> p integer(c_int) flags \[dimension(3)\]
>
> :   three-element vector where the decoded image flags will be stored.

------------------------------------------------------------------------

> Set the callback function for a [fix external](fix_external) instance
> with the given ID.
>
> ::: versionadded
> 22Dec2022
> :::
>
> Fix [external](fix_external) allows programs that are running LAMMPS
...
>
> This subroutine sets the callback function for use with the
> \"pf/callback\" mode. The function should have Fortran language
> bindings with the following interface, which depends on how LAMMPS was
> compiled:
>
> ``` fortran
> ABSTRACT INTERFACE
...
>
> The argument *caller* to `set_fix_external_callback`{.interpreted-text
> role="f:subr"} is unlimited polymorphic (i.e., it can be any Fortran
> object you want to pass to the calling function) and will be available
> as the first argument to the callback function. It can be your LAMMPS
> instance, which you might need if the callback function needs access
> to the library interface. The argument must be a scalar; to pass
> non-scalar data, wrap those data in a derived type and pass an
> instance of the derived type to *caller*.
>
> The array *ids* is an array of length *nlocal* (as accessed from the
> `Atom`{.interpreted-text role="cpp:class"} class or through
> `extract_global`{.interpreted-text role="f:func"}). The arrays *x* and
> *fexternal* are $3 \times {}$\*nlocal\* arrays; these are transposed
> from what they would look like in C (see note about array index order
> at `extract_atom`{.interpreted-text role="f:func"}).
>
> The callback mechanism is one of two ways that forces can be applied
> to a simulation with the help of [fix external](fix_external). The
> alternative is *array* mode, where one calls
> `fix_external_get_force`{.interpreted-text role="f:func"}.
>
> Please see the documentation for [fix external](fix_external) for more
> information about how to use the fix and couple it with external
> programs.
>
> p character(len=\*) id
>
> :   ID of [fix external](fix_external) instance
>
> p callback
>
> :   subroutine [fix external](fix_external) should call
>
> ptype callback
>
> :   external
>
> p class(\*) caller \[optional\]
>
> :   object you wish to pass to the callback procedure (must be a
>     scalar; see note)
>
> to
>
...
> get a segmentation fault or at least a misinterpretation of whatever
> is in memory there. You can resolve the object using the `SELECT TYPE`
...
> Get pointer to the force array storage in a fix external instance with
> the given ID.
>
> ::: versionadded
> 22Dec2022
...
>
> Fix [external](fix_external) allows programs that are running LAMMPS
> through its library interfaces to add or modify certain LAMMPS
> properties on specific time steps, similar to the way other fixes do.
>
...
> `extract_atom`{.interpreted-text role="f:func"} function. Please note
> that the array stores the forces for *local* atoms for each MPI rank,
> in the order determined by the neighbor list build. Because the
> underlying data structures can change as well as the order of atom as
> they migrate between MPI processes because of the domain decomposition
...
> You can use, for example, `extract_setting`{.interpreted-text
> role="f:func"} to obtain the number of local atoms
> [nlocal]{.title-ref} and then assume the dimensions of the returned
> force array as `REAL(c_double) :: force(3,nlocal)`.
>
> This function is an alternative to the callback mechanism in fix
> external set up by `set_fix_external_callback`{.interpreted-text
> role="f:subr"}. The main difference is that this mechanism can be used
> when forces are to be pre-computed and the control alternates between
> LAMMPS and the external driver, while the callback mechanism can call
> an external subroutine to compute the force when the fix is triggered
> and needs them.
>
> Please see the documentation for [fix external](fix_external) for more
> information about how to use the fix and how to couple it with an
> external program.
>
> p character(len=\*) id
>
...
> r fexternal
>
> :   pointer to the per-atom force array allocated by the fix
>
> rtype fexternal
>
> :   real(c_double),dimension(3,nlocal)

------------------------------------------------------------------------

> Set the global energy contribution for a [fix external](fix_external)
...
> `set_fix_external_callback`{.interpreted-text role="f:func"} and
> `fix_external_get_force`{.interpreted-text role="f:func"} that also
> sets the contribution to the global energy from the external program.
> The value of the *eng* argument will be stored in the fix and applied
> on the current and all following time steps until changed by another
> call to this function. The energy is in energy units as determined by
> the current [units](units) settings and is the **total** energy of the
> contribution. Thus, when running in parallel, all MPI processes have
> to call this function with the **same** value, and this will be
> returned as a scalar property of the fix external instance when
> accessed in LAMMPS input commands or from variables.
>
> Please see the documentation for [fix external](fix_external) for more
> information about how to use the fix and how to couple it with an
> external program.
...
> p character(len=\*) id
>
> :   fix ID of fix external instance
>
> p real(c_double) eng
>
> :   total energy to be added to the global energy
>
> to
>
...
> Set the global virial contribution for a fix external instance with
> the given ID.
>
> ::: versionadded
> 22Dec2022
...
>
> This is a companion function to
> `set_fix_external_callback`{.interpreted-text role="f:subr"} and
> `fix_external_get_force`{.interpreted-text role="f:func"} to set the
> contribution to the global virial from an external program.
>
> The six values of the *virial* array will be stored in the fix and
> applied on the current and all following time steps until changed by
> another call to this function. The components of the virial need to be
> stored in the following order: *xx*, *yy*, *zz*, *xy*, *xz*, *yz*. In
> LAMMPS, the virial is stored internally as
> [stress\*volume]{.title-ref} in units of
> [pressure\*volume]{.title-ref} as determined by the current
> [units](units) settings and is the **total** contribution. Thus, when
> running in parallel, all MPI processes have to call this function with
> the **same** value, and this will then be added by fix external.
>
> Please see the documentation for [fix external](fix_external) for more
> information about how to use the fix and how to couple it with an
> external code.
...
> :   fix ID of fix external instance
>
> p real(c_double) virial \[dimension(6)\]
>
> :   the six global stress tensor components to be added to the global
>     virial
>
> to
>
...
> Set the per-atom energy contribution for a fix external instance with
> the given ID.
>
> ::: versionadded
> 22Dec2022
...
>
> This is a companion function to
> `set_fix_external_callback`{.interpreted-text role="f:subr"} to set
> the per-atom energy contribution due to the fix from the external
> program as part of the callback function. For this to work, the LAMMPS
> object must be passed as part of the *caller* argument when
> registering the callback function, or the callback function must
> otherwise have access to the LAMMPS object, such as through a
> module-based pointer.
>
> :::: note
> ::: title
> Note
...
> `fix_external_set_energy_global`{.interpreted-text role="f:subr"} and
> will **NOT** add any contributions to the global energy tally and will
> **NOT** check whether the sum of the contributions added here are
...
> p character(len=\*) id
>
> :   fix ID of the fix external instance
>
> <!-- -->
>
...
> This is a companion function to
> `set_fix_external_callback`{.interpreted-text role="f:subr"} to set
> the per-atom virial contribution due to the fix from the external
> program as part of the callback function. For this to work, the LAMMPS
> object must be passed as the *caller* argument when registering the
> callback function.
>
> ::: versionadded
> 22Dec2022
...
>
> This function is fully independent from
> `fix_external_set_virial_global`{.interpreted-text role="f:subr"} and
> will **NOT** add any contributions to the global virial tally and
> **NOT** check whether the sum of the contributions added here are
> consistent with the global added virial.
> ::::
>
> The order and units of the per-atom stress tensor elements are the
> same as for the global virial. The type and dimensions of the per-atom
> virial array must be `REAL(c_double), DIMENSION(6,nlocal)`.
>
> Please see the documentation for [fix external](fix_external) for more
> information about how to use the fix and how to couple it with an
> external program.
...
> :   fix ID of fix external instance
>
> :p real(c_double) virial \[dimension(:,:)\]: an array of $6 \times{}$
> *nlocal* components to be added to the per-atom virial :to:
> `lammps_set_virial_peratom`{.interpreted-text role="cpp:func"}
...
> `set_fix_external_callback`{.interpreted-text role="f:subr"} and
> `fix_external_get_force`{.interpreted-text role="f:func"} to set the
> length of a global vector of properties that will be stored with the
> fix via `fix_external_set_vector`{.interpreted-text role="f:subr"}.
>
> This function needs to be called **before** a call to
> `fix_external_set_vector`{.interpreted-text role="f:subr"} and
> **before** a run or minimize command. When running in parallel, it
> must be called from **all** MPI processes with the same length
> argument.
>
> Please see the documentation for [fix external](fix_external) for more
> information about how to use the fix and how to couple it with an
...
> :   fix ID of fix external instance
>
> p integer(c_int) length
>
> :   length of the global vector to be stored with the fix
>
> to
>
...
> Store a global vector value for a fix external instance with the given
> ID.
>
> ::: versionadded
> 22Dec2022
...
> `set_fix_external_callback`{.interpreted-text role="f:subr"} and
> `fix_external_get_force`{.interpreted-text role="f:func"} to set the
> values of a global vector of properties that will be stored with the
> fix and can be accessed from within LAMMPS input commands (e.g., fix
> ave/time or variables) when used in a vector context.
>
> This function needs to be called **after** a call to
> `fix_external_set_vector_length`{.interpreted-text role="f:subr"} and
> **before** a run or minimize command. When running in parallel, it
> must be called from **all** MPI processes with the **same** *idx* and
> *val* parameters. The variable *val* is assumed to be extensive.
>
> :::: note
> ::: title
> Note
...
> is set with *idx*${} = 1$, and the last element of the vector with
> *idx*${} = N$, where $N$ is the value of the *length* parameter of the
> call to `fix_external_set_vector_length`{.interpreted-text
> role="f:subr"}).
> ::::
>
> Please see the documentation for [fix external](fix_external) for more
...
> p character(len=\*) id
>
> :   ID of fix external instance
>
> p integer(c_int) idx
>
> :   1-based index in global vector
>
> p integer(c_int) val
>
> :   value to be stored in global vector at index *idx*
>
> to
>
> :   `lammps_fix_external_set_vector`{.interpreted-text
>     role="cpp:func"}

------------------------------------------------------------------------

> This function calls `lammps_flush_buffers`{.interpreted-text
> role="cpp:func"}, which flushes buffered output to be written to
> screen and logfile. This can simplify capturing output from LAMMPS
> library calls.
>
> ::: versionadded
> 3Nov2022
...
> r is_running
>
> :   `.FALSE.` if idle or `.TRUE.` if active
>
> rtype is_running
>
> :   logical
...
> :::
>
> This function can be used to query if an error inside of LAMMPS has
> thrown a [C++ exception](exceptions).
>
> :::: note
> ::: title
> Note
...
> function `lammps_config_has_exceptions`{.interpreted-text
> role="cpp:func"} to check if this is the case.
> ::::
>
> to
>
...
> in the event of an error inside of LAMMPS that resulted in a [C++
> exception](exceptions). A suitable buffer for a string has to be
> provided. If the internally-stored error message is longer than the
> string, it will be truncated accordingly. The optional argument
> *status* indicates the kind of error: a \"1\" indicates an error that
> occurred on all MPI ranks and is often recoverable, while a \"2\"
> indicates an abort that would happen only in a single MPI rank and
> thus may not be recoverable, as other MPI ranks may be waiting on the
> failing MPI rank(s) to send messages.
>
> :::: note
> ::: title
> Note
...
> `config_has_exceptions`{.interpreted-text role="f:func"} to check
> whether this is the case.
> ::::
>
> p character(len=\*) buffer
>
> :   string buffer to copy the error message into
>
> o integer(c_int) status \[optional\]
>
