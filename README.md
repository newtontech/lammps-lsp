...
A language server, diagnostic CLI, and agent CLI for LAMMPS input scripts.

`lammps-lsp` is the maintained `newtontech/lammps-lsp` fork of
`chappertron/lammps-analyser`. It preserves the upstream checker and legacy
`lmp-lsp` command while adding the standard `lammps-lsp` server alias and the
JSON-first `lammps-lsp-tool` agent interface.
...
Candidate `v0.1.0-pre.4` is configured for GitHub Releases only. After that tag
is published, the cargo-dist shell installer selects the matching macOS or
Linux archive:

``` bash
curl --proto '=https' --tlsv1.2 -LsSf \
  https://github.com/newtontech/lammps-lsp/releases/download/v0.1.0-pre.4/lammps-lsp-installer.sh | sh
```

For Windows, use the PowerShell installer:

``` powershell
powershell -ExecutionPolicy ByPass -c "irm https://github.com/newtontech/lammps-lsp/releases/download/v0.1.0-pre.4/lammps-lsp-installer.ps1 | iex"
```

The locked candidate matrix contains archives for Apple Silicon macOS, Intel
macOS, x86_64 Linux, and x86_64 Windows. Each archive includes
`lammps-analyser`, `lammps-lsp`, `lammps-lsp-tool`, and the legacy `lmp-lsp`
alias. The release workflow publishes a checksum beside every archive and does
not publish this prerelease to crates.io.

### Installing from source with Cargo

Until a separate crates.io release is approved, install directly from the
tagged GitHub source:

``` sh
cargo install --locked --git https://github.com/newtontech/lammps-lsp \
  --tag v0.1.0-pre.4 lammps-lsp
```
...
The language server and agent-facing commands are:

```bash
lammps-lsp
lmp-lsp                  # compatibility alias
lammps-lsp-tool --help
lammps-lsp-tool check tests/fixtures/valid/minimal.in --fail-on-blocking
```

Every shipped command reports the same release identity:

```bash
lammps-lsp --version
# lammps-lsp 0.1.0-pre.4
```
